...
<div class="login">
	<form action="doReg.php" method="post">
		<table class="table" style="border-width: 1px;" cellspacing="1" cellpadding="5" border="0" align="center" width="100%">
			...
			<tr height="60px">
				<td width="20%" align="center">用户邮箱</td>
				<td width="40%" align="center"><input type="test" name="email" id="email" onmouseout="testIfTrue(2)"></td>
				<td width="40%" align="left"><input id="ifTrue_3" value="" style="display: block;color:red;font-size:15px;border-width:0;"></input></td>
			</tr>
			<tr height="60px">
			...
			<tr height="60px">
				<div width="60%"><button class="log" style="color:red;width:60%;position:absolute;bottom:145px;height:40px;font-size:20px;font-family:'楷体'">登录</button></div>
			</tr>
		</table>
	</form>
</div>
...
function testIfTrue(num){
	if(num == 0){
		...
		if(test.value.match(pattern)){
			test_play.value="用户名可用";
			test_play.style.color="green";
		}else{
			test_play.value="用户名不可用";
			test.value="";
		}
	}
	else if(num == 1){
		...
		var test_play = document.getElementById("ifTrue_3");
		test_play.style.color="red";
		var pattern = /(\d|\w)+@(((\w+)\.)+)((com\.cn)|com|cn){1}/;
		if(result = (test.value.match(pattern))){
			test_play.value="邮箱可用";
			test_play.style.color="green";
		}else{
			test_play.value="邮箱格式错误";
			test.value="";
		}
	}

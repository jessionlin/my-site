								...
								<table  class="table" style="border-width: 1px;" cellspacing="0" cellpadding="5" border="1" align="center">
									...
									<tbody>
										<?php $link = mysqli_connect(DB_HOST,DB_USER,DB_PWD,DB_DBNAME);
												$sql="select name,id,createtime,details from createadv";
												...
												$sql="select name,id,createtime,details from createadv limit {$offset},{$pageSize}";
...
<script>
	function ifDes(id){
		if(window.confirm("您确认要删除嘛？")){
			window.location='delMes.php?table=createadv&target=createAdv&id='+id;
		}
	}
</script>
</html>